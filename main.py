...
async def webhook(request: Request):
    ...
    if msg and "text" in msg:
        ...
        if text.startswith("/start"):
            ...
            await send_text(
                chat_id,
                f"Начало игры. У тебя {s.hp}/{s.max_hp} жизней. "
                f"Уровень: {s.level}. Урон: {s.dmg_min}-{s.dmg_max}."
            )
            return {"ok": True}
        ...
        if text in ("/сброс", "/reset"):
            SESS[chat_id] = Session()
            s = sget(chat_id)
            await send_text(chat_id, f"Прогресс сброшен. Жизни восстановлены: {s.hp}/{s.max_hp}. Уровень: {s.level}.")
            return {"ok": True}
    ...
    if cq:
        ...
        if data == "fight:potion":
            if have(s, "зелье"):
                heal(s, 15)  # на больших хп чуть полезнее
                s.inventory = [x for x in s.inventory if norm(x) != "зелье"]
                await send_text(chat_id, f"🍵 Ты выпил зелье. Жизни: {s.hp}/{s.max_hp}")
            else:
                await send_text(chat_id, "Зелья нет.")
            return {"ok": True}
        ...
        if data == "fight:win":
            level_up(s)
            await send_text(chat_id, f"🏆 Победа! Новый уровень: {s.level}. "
                                     f"HP: {s.hp}/{s.max_hp}. Урон: {s.dmg_min}-{s.dmg_max}.")
            return {"ok": True}
...
async def webhook(request: Request):
    ...
    if msg and "text" in msg:
        ...
        if text.startswith("/start"):
            SESS[chat_id] = Session()
            await send_text(chat_id, "Начало игры. У тебя 8 жизней.")
            return {"ok": True}
        ...
        if text in ("/сброс", "/reset"):
            SESS[chat_id] = Session()
            await send_text(chat_id, "Прогресс сброшен. Жизни восстановлены.")
            return {"ok": True}
    ...
    if cq:
        ...
        if data == "fight:win":
            level_up(s)
            await send_text(chat_id, f"🏆 Победа! Твои жизни прокачаны: {s.hp}/{s.max_hp}")
            return {"ok": True}
